    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            for j in range(batch_size):
                _, label_list = loader.dataset.get_label(item[j].item())
                corners, scores = preds_filtered[j]
                gts += len(label_list)
                preds += len(scores)
                all_scores.extend(list(scores))
                if item[j] in log_img_list:
                    bev_np = bev[j].cpu().permute(1, 2, 0).numpy()
...
def eval_one(net, loss_fn, config, loader, image_id, device, plot=False, verbose=False):
    ...
    if plot == True:
        ...
        plot_bev(bev_np, corners, window_name='Prediction')
